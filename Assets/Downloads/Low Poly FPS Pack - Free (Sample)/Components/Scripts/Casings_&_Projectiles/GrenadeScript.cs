...
public class GrenadeScript : MonoBehaviour {
	...
	private IEnumerator ExplosionTimer () 
	{
		...
		foreach (Collider hit in colliders) {
			...
			if (hit.GetComponent<Collider>().tag == "Target" && hit.gameObject.GetComponent<TargetScript>().isHit == false) 
			{
				//Animate the target 
			...
			//If the explosion hits an Enemy
			if(hit.gameObject.tag == "Enemy")
            {
				hit.gameObject.GetComponent<ZombieBehaviour>().ReceiveDamage(3);
            }
